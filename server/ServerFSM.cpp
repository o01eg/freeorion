...
sc::result MPLobby::react(const JoinGame& msg) {
    ...
    if (server.Networking().CheckCookie(cookie, player_name, roles, authenticated)) {
        ...
        const bool relaxed_auth = player_connection->IsLocalConnection() && client_type == Networking::CLIENT_TYPE_AI_PLAYER;
...
sc::result WaitingForTurnEnd::react(const CheckTurnEndConditions& c) {
        ...
        !GetOptionsDB().Get<bool>("network.server.turn-timeout.fixed-interval")))
    {
        ...
        for (int last_empire_id : last_empires_id) {
            ...
            if (empire != nullptr &&
                !empire->Eliminated() &&
                server.GetEmpireClientType(empire->EmpireID()) == Networking::INVALID_CLIENT_TYPE)
            {
