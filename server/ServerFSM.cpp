        ...
        for (const auto& player_connection : server.m_networking.AllPlayerConnections()) {
            ...
            if (!Networking::is_ai(player_connection)) {
                ...
                const auto& species_name = m_lobby_data->seed.empty() ?
                    sm.SequentialPlayableSpeciesName(player_id) : "RANDOM";
                ...
                if (m_ai_next_index <= max_ai || max_ai < 0) {
                    ...
                    const auto& species_name = m_lobby_data->seed.empty() ?
                        sm.SequentialPlayableSpeciesName(m_ai_next_index) : "RANDOM";
                    m_lobby_data->players.emplace_back(
                        std::piecewise_construct,
                        std::forward_as_tuple(Networking::INVALID_PLAYER_ID),
                        std::forward_as_tuple(std::move(player_name),
                              ...
                              std::string client_version_string, Networking::AuthRoles roles)
{
    ...
    const auto& species_name = m_lobby_data->seed.empty() ?
        "RANDOM" : sm.SequentialPlayableSpeciesName(player_id);
    m_lobby_data->players.emplace_back(
        std::piecewise_construct,
        std::forward_as_tuple(player_id),
            ...
            if (Networking::is_mod_or_obs(psd)) {
                ...
                if (psd.starting_species_name.empty()) {
                    if (!m_lobby_data->seed.empty())
                        psd.starting_species_name = "RANDOM";
                    else
    ...
    if (m_expected_ai_player_names.empty()) {
        ...
        if (m_player_save_game_data.empty()) {
            ...
            for (const auto& empire : Empires()) {
                if (server.GetEmpireClientType(empire.first) == Networking::ClientType::INVALID_CLIENT_TYPE &&
...
sc::result WaitingForTurnEnd::react(const CheckTurnEndConditions& c) {
        ...
        !GetOptionsDB().Get<bool>("network.server.turn-timeout.fixed-interval")))
    {
        ...
        if (GetOptionsDB().Get<bool>("network.server.send-turn-chat")) {
            // list players who missed turn
            std::set<int> last_empires_id = server.LastNotReadyEmpires();
            std::string last_player_names{"Slowpokes:\n"};
            bool has_slowpokes = false;
            for (int last_empire_id : last_empires_id) {
                const Empire* empire = server.GetEmpire(last_empire_id);
                if (empire != nullptr &&
                    !empire->Eliminated() &&
                    server.GetEmpireClientType(empire->EmpireID()) == Networking::ClientType::INVALID_CLIENT_TYPE)
                {
