...
sc::result MPLobby::react(const Disconnection& d) {
    ...
    if (const bool player_was_in_lobby = (id_it != m_lobby_data->players.end())) {
        ...
        std::async(std::launch::async, [players_size, &server] {
            std::vector<std::string> args{"/usr/bin/curl",
                "http://localhost:8083/",
                "-H", "X-XMPP-Muc: smac",
                "-d", " @freeorion in lobby " + std::to_string(players_size)};
            Process sendxmpp = Process(server.m_networking.IoContext(), "/usr/bin/curl", args);
                              ...
                              std::string client_version_string, Networking::AuthRoles roles)
{
    ...
    std::async(std::launch::async, [player_name, players_size, &server] {
        std::vector<std::string> args{"/usr/bin/curl",
            "http://localhost:8083/",
            "-H", "X-XMPP-Muc: smac",
